...
export default function Dashboard() {
  ...
  const handleDelete = async (team) => {
    const typed = prompt(
  `⚠️ PERINGATAN\n\nKetik nama tim persis:\n${team.name}\n\nuntuk menghapus permanen`
);

    if (!typed) return;

    if (typed.trim().toLowerCase() !== team.name.trim().toLowerCase()) {
      alert("Nama tim tidak sesuai ❌");
      return;
    }
      ...
      >
        {active ? (
          <div className="max-w-4xl w-full mx-auto pb-24 md:p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={field} className="space-y-1">
                        ...
                        <div className="flex gap-2 mt-3 flex-wrap">
                          {active.managerPhoto && (
                            <img
                              ...
                              className="w-20 h-28 md:w-24 md:h-32 object-cover rounded-md border cursor-pointer"
                            />
                          )}
                          ...
                          {active.managerKtp && (
                            <img
                              ...
                              className="w-24 h-16 md:w-32 md:h-20 object-cover rounded-md border cursor-pointer"
                            />
                          )}
                        </div>
                      )}
                      ...
                      {/* OFFICIAL 1 */}
                      {field === "official1" && active.official1Photo && (
                        <img
                          ...
                          className="w-20 h-28 object-cover rounded-md border cursor-pointer mt-3"
                        />
                      )}

                      {/* OFFICIAL 2 */}
                      {field === "official2" && active.official2Photo && (
                        <img
                      ...
                      {/* OFFICIAL 3 */}
                      {field === "official3" && active.official3Photo && (
                        <img
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {active.players?.map((p, i) => (
                    <div
                      key={p.id}
                      className="bg-white border border-gray-100 p-4 md:p-5 rounded-2xl shadow-sm space-y-4 relative group"
                    >
            ...
            {/* FLOATING ACTION BAR */}
            <div className="fixed bottom-0 left-0 right-0 md:left-auto md:right-8 md:bottom-8 p-3 md:p-4 bg-white/90 md:bg-transparent backdrop-blur-md md:backdrop-blur-none border-t md:border-none z-20">
              <div className="max-w-4xl mx-auto flex gap-3">
                ...
                <button
                  onClick={handleSave}
                  className="flex-[2] md:flex-none px-6 md:px-10 py-3 bg-red-600 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 shadow-red-200 shadow-xl transition-transform active:scale-95 hover:bg-red-700"
                >
